...
# 1. Replace the Column with LazyColumn
old_column = """        Column(
            modifier = Modifier
                .fillMaxSize()
                    ...
                    selectedRival?.let { rival ->
                        item {
                            RivalActionPanel(
                                ...
                                onProposeTradeDeal = { 
                                    viewModel.proposeTradeDeal(rival.id, TradeCommodity.GRAIN, 100L, TradeType.EXPORT)
                                    Toast.makeText(context, "Trade deal proposed.", Toast.LENGTH_SHORT).show()
                                },
                                onSendAid = { 
                                    viewModel.sendForeignAid(rival.id)
                                    Toast.makeText(context, "Foreign aid sent.", Toast.LENGTH_SHORT).show()
                                },
                                onStateVisit = { 
                                ...
                                onNegotiateTradeTreaty = { 
                                    viewModel.negotiateTreaty(rival.id, TreatyType.TRADE)
                                    Toast.makeText(context, "Trade treaty signed.", Toast.LENGTH_SHORT).show()
                                },
                                onNegotiateNonAggression = { 
                                    viewModel.negotiateTreaty(rival.id, TreatyType.NON_AGGRESSION)
                                    Toast.makeText(context, "Non-aggression signed.", Toast.LENGTH_SHORT).show()
                                },
                                onFormAlliance = { 
                                    viewModel.formAlliance("Pact with ${rival.name}", listOf(rival.id))
                                    ...
                                    if (rival.hasTradeTreaty) {
                                        OutlinedButton(
                                            onClick = { 
                                                ...
                                                Toast.makeText(context, "Trade treaty broken.", Toast.LENGTH_SHORT).show()
                                            },
                                            modifier = Modifier.fillMaxWidth(),
                                        ) {
                                            Text("Break Trade")
                                    ...
                                    if (rival.hasNonAggressionPact) {
                                        OutlinedButton(
                                            onClick = { 
                                                ...
                                                Toast.makeText(context, "Non-Aggression broken.", Toast.LENGTH_SHORT).show()
                                            },
                                            modifier = Modifier.fillMaxWidth(),
                                            colors = ButtonDefaults.outlinedButtonColors(contentColor = NssRed),
                                        ) {
...
new_negotiations = """                "NEGOTIATIONS" -> {
                    if (state.economy.tradeDeals.isEmpty()) {
                        item { Text("No active trade negotiations.", color = NssMutedForeground) }
                    } else {
                        items(state.economy.tradeDeals) { deal ->
                            NssCard {
                                Text("${deal.type} — ${deal.commodity}", fontWeight = FontWeight.Bold, color = NssForeground)
                                Text("Partner: ${deal.partnerCountryId}", color = NssMutedForeground, fontSize = 12.sp)
                                Text(
